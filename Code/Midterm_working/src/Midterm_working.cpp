...
void loop(){
 ...
 newPosition = myEnc.read()/4.0;//new position set by reading the encoder, divided by four to synch one click per choice 
 if(newPosition<=0){
  newPosition=0;
 }
...
case 4: //MENU 4 ALL ON/OFF/RETURN
  if (newPosition %3 ==0){//ALL on 
    ...
    if (encoderButt==1){
      ...
      setHue(hueNum,true,0x008080,255,255);
      delay(500);
      }
    }
  }
  if (newPosition %3 ==1){//ALL OFF 
    ...
    display.printf("\nReturn");
     ...
     if (encoderButt==1){
      ...
      setHue(hueNum,false,0x008080,255,255);
      delay(500);
      }
     }
    //ADD ALL OFF FUNCTION 
    }
